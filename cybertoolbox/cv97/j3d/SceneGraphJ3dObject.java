...
public class SceneGraphJ3dObject extends Object implements cv97.SceneGraphObject {
	...
	private void initialize(Canvas3D canvas3D[], SceneGraph sg) {
...
*/
	...
	public boolean addNode(SceneGraph sg, cv97.node.Node node) {
	...
	public boolean removeNode(SceneGraph sg, cv97.node.Node node) {
	...
	public boolean initialize(SceneGraph sg) {
		...
		for (cv97.node.Node node = sg.getNodes(); node != null; node = node.nextTraversal()) {
...
*/
	...
	public Shape3D pickShape3D(Canvas3D canvas3D, int mx, int my) {
		PickObject pickObject = new PickObject(canvas3D, mRootNode);
		SceneGraphPath sgPath = pickObject.pickClosest(mx, my, PickObject.USE_GEOMETRY);

		if (sgPath == null)
			return null;
		Shape3D shape3d = (Shape3D)pickObject.pickNode(sgPath, PickObject.SHAPE3D, 1);
