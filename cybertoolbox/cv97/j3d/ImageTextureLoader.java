...
public class ImageTextureLoader extends Object {
	...
	private void loadImageComponent(ImageTextureNode imgTex, Component comp) {
		...
		cv97.util.Debug.warning("Image URL = " + url);
		
		if (image == null) {
			SceneGraph sg = imgTex.getSceneGraph();
			if (sg != null) {
				URL baseURL = sg.getBaseURL();
				cv97.util.Debug.warning("Base URL = " + baseURL);
				if (baseURL != null) {
					try {
						URL imgURL = new URL(baseURL.toString() + url);
						cv97.util.Debug.warning("Loading Texture (" + imgURL + ") .....");
		...
		if (image == null) {
			...
			cv97.util.Debug.warning("Texture (" + url + ") is not found");
			return;
		}
