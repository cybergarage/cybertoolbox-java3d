...
public class VRML97Saver extends cv97.SceneGraph {
	...
	public void addJ3dGroupNode(Group j3dGroup) {
		...
		cv97.node.Node vrml97Node = addJ3dNode(j3dGroup);
		...
		for (int n=0; n<j3dGroup.numChildren(); n++) {
	...
	public cv97.node.Node addJ3dNode(javax.media.j3d.Node j3dNode) {
		cv97.node.Node vrml97Node = createVRML97Node(j3dNode);
		if (vrml97Node != null)
	...
	public void pushVRML97Node(cv97.node.Node node) {
		mVRML97NodeStack.push(node);
	}

	public cv97.node.Node popVRML97Node() {
		return (cv97.node.Node)mVRML97NodeStack.pop();
	}

	public cv97.node.Node getCurrentVRML97Node() {
		cv97.node.Node stackNode = null;
		try {
			stackNode = (cv97.node.Node)mVRML97NodeStack.peek();
		}
	...
	public cv97.node.Node createVRML97Node(javax.media.j3d.Node j3dNode) {
	...
	public ShapeNode createVRML97ShapeNode(Shape3D j3dNode) 
	{
		...
		if (j3dGeom != null) {
			GeometryNode geomNode = createVRML97GeometryNode(j3dGeom); 
			if (geomNode != null)
	...
	public GeometryNode createVRML97GeometryNode(Geometry j3dNode) {
		
		GeometryNode vrml97Node = null;
	...
	public IndexedFaceSetNode createVRML97IndexedFaceSetNode(QuadArray j3dNode) {
		...
		if (addGeometryArrayCoordinateToIndexedFaceSetNode(vrml97Node, j3dNode) == true) {
			for (int n=0; n<j3dNode.getVertexCount(); n=+4) {
				vrml97Node.addCoordIndex(n);
				vrml97Node.addCoordIndex(n+1);
				vrml97Node.addCoordIndex(n+2);
