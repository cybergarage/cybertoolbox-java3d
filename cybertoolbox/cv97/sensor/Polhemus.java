...
public abstract class Polhemus extends SerialPort {
	...
	public static final String activeStationStateCommand	= "l1\r";	
	...
	private void readData() {
		...
		for (int n=0; n<getActiveReceivers(); n++) {
			...
			if (data.length == msgLength) {
				for (i=0; i<3; i++) {
					Float value = new Float(new String(data, posOffset+(7*i), 7));
					mPositionData[n][i] = value.floatValue();
				}
				for (i=0; i<3; i++) {
					Float value = new Float(new String(data, rotOffset+(7*i), 7));
